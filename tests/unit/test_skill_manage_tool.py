...
# ── Prompt contribution (Cluster 5 / E.1 consumer) ────────────────────


def test_prompt_contribution_nudges_save_and_patch(tmp_path: Path) -> None:
    """The system-prompt contribution mentions both create/patch routes
    and the ``##skill`` / ``/<name>`` invocation paths."""
    tool = _make_tool(tmp_path)

    assert tool.prompt_contribution_bucket == "first"

    text = tool.prompt_contribution()
